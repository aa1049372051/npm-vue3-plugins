...
var util = (Crypto.util = {
  setOss: function (ossData) {
    let accessid = ossData.accessKeyId || '';
    let accesskey = ossData.accessKeySecret;
    let host = `https://${ossData.bucketName}.${ossData.endpoint}`;
    let date = new Date(new Date(new Date().getTime() + 3600000 * 24));//24小时后过期
    date.setHours(
      date.getHours(),
      date.getMinutes() - date.getTimezoneOffset()
    );
    var policyText = {
      // "expiration": "2020-06-01T12:00:00.000Z", //设置该Policy的失效时间，超过这个失效时间之后，就没有办法通过这个policy上传文件了
