...
instance.interceptors.request.use(
  (config) => {
    ...
    //   config["headers"]["Authorization"] = "Bearer " + store.tokenInfo;
    // }
    return config;
  },
  (error) => {
...
instance.defaults.transformRequest = [
  function (data) {
    ...
    post.channel = store.channel
    post.spCode = store.spCode
    post.accessToken = store.accessToken
    post.userId = store.userId
    post.organId = store.organId
    post.param = data;
    //统一采用qs发送数据
...
instance.interceptors.response.use(
  ...
  (response) => {
    ...
    if (
      response.data &&
      response.data &&
      "300" == response.data.retCode &&
      "您的登录状态已失效，请重新登录" == response.data.retMsg
    ) {
      toLogin();
    }
    return response;
